...
// DexScreener 代币图标
function TokenLogo({ chain, tokenAddress, symbol, size = 20 }) {
  const [src, setSrc] = useState(null)
  const [failed, setFailed] = useState(false)

  useEffect(() => {
    if (!tokenAddress || failed) return
    // DexScreener API: chain 名称需要转换
    const chainMap = { bsc: 'bsc', solana: 'sol', eth: 'eth', base: 'base' }
    const apiChain = chainMap[(chain || '').toLowerCase()] || 'bsc'
    const url = `https://api.dexscreener.com/token-icons/${apiChain}/${tokenAddress}`
  ...
  if (failed || !src) {
    return <Identicon seed={tokenAddress} size={size} />
  }

  return (
    <img
      src={src}
      alt={symbol || tokenAddress}
      width={size}
      height={size}
      ...
      {unique.map((r, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-1 bg-dark-700/40 rounded px-1.5 py-0.5">
            <TokenLogo chain={r.chain} tokenAddress={r.token} symbol={r.symbol} size={16} />
            ...
            {item.records.map((r, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-1.5 bg-dark-700/60 border border-dark-600 rounded px-2 py-1">
                  <TokenLogo chain={r.chain} tokenAddress={r.token} symbol={r.symbol} size={18} />
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-dark-700/40 rounded-lg px-3 py-2.5">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs">
            ...
            <span className="text-gray-400">胜率 <span className="text-indigo-400 font-medium">{item.win_rate}</span></span>
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          <div onClick={e => e.stopPropagation()}>
            {isFollowing ? (
              ...
              <button onClick={() => onFollowClick(item)}
                className="text-[11px] px-2 py-0.5 rounded border border-accent-indigo-40 text-indigo-400 bg-accent-indigo-10 font-medium">
                +跟单
              </button>
            )}
          </div>
      ...
      <tr className={clsx('hidden md:table-row border-b border-dark-700/40 transition-colors cursor-pointer', expanded ? 'bg-dark-700/30' : 'hover:bg-white/[0.025]')} onClick={() => onDetailClick(item)}>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-0.5">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-xs text-indigo-400 font-medium">🔗 {fs.follow_count}次</span>
        ...
        <td className="px-2 py-4 w-20 text-center" onClick={e => e.stopPropagation()}>
          {isFollowing ? (
            ...
            <button onClick={() => onFollowClick(item)} className="text-xs px-2.5 py-1.5 rounded border border-accent-indigo-40 text-indigo-400 bg-accent-indigo-10 hover:bg-accent-indigo-20 hover:border-accent-indigo-70 transition-colors whitespace-nowrap font-medium">
              + 跟单
            </button>
          )}
        </td>
          ...
          <div className="bg-dark-800 border border-dark-600 rounded-xl w-full max-w-sm mx-4 shadow-2xl">
            ...
            <div className="px-5 py-4 border-t border-dark-600 flex gap-3 justify-end">
              ...
              <button
                ...
                className="text-xs px-4 py-2 rounded border border-accent-indigo-50 text-indigo-400 bg-accent-indigo-10 hover:bg-accent-indigo-20 font-medium transition-colors"
              >确认跟单</button>
            </div>
          </div>
        </div>
            ...
            <button
              ...
              className={clsx(
                ...
                batchFollowing
                  ...
                  : 'border-accent-indigo-50 text-indigo-400 bg-accent-indigo-10 hover:bg-accent-indigo-20'
              )}
            >
