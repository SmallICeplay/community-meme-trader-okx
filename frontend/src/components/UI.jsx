...
export function PnlValue({ value, suffix = 'U' }) {
  const isPos = value >= 0
  return (
    <span className={isPos ? 'text-indigo-400 font-mono' : 'text-accent-red font-mono'}>
...
export function Button({ children, onClick, disabled, variant = 'primary', className, size = 'md' }) {
  const variants = {
    primary: 'bg-dark-600 hover:bg-dark-500 text-gray-200 border border-dark-500',
    ...
    <label className="flex items-center gap-3 cursor-pointer select-none">
      <div
        ...
        className={clsx(
          ...
          checked ? 'bg-gray-500' : 'bg-dark-500'
        )}
      >
        <div className={clsx(
...
export function StatCard({ label, value, sub, color = 'white', index = 0, winRate }) {
  const colors = { white: 'text-white', green: 'text-indigo-400', red: 'text-accent-red', yellow: 'text-accent-yellow' }
