    ...
    <div className="p-3 space-y-3 text-xs">
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <div
          ...
          className={clsx('w-8 h-4 rounded-full relative transition-colors', enabled ? 'bg-indigo-500' : 'bg-dark-500')}
        >
      ...
      <div className="text-[10px] text-gray-500 bg-dark-700 border border-dark-600 rounded px-2 py-1.5">
        API Key、模型选择、内置Key开关 → 请前往 <span className="text-indigo-400">配置</span> 页面 → AI 助手接口
      </div>
      ...
      <button
        onClick={save}
        disabled={saving}
        className="w-full py-1.5 rounded bg-accent-indigo-80 hover:bg-indigo-500 text-white text-xs font-semibold transition-colors"
      >
        {saving ? '保存中...' : '保存'}
      </button>
    </div>
            ...
            <>
              ...
              <button
                ...
                className={clsx('text-[10px] px-1.5 py-0.5 rounded border transition-colors',
                  showSettings
                    ? 'border-accent-indigo-50 text-indigo-400 bg-accent-indigo-10'
                ...
                {messages.map((m, i) => (
                  <div
                    ...
                    className={clsx(
                      ...
                      m.role === 'user'
                        ? 'ml-auto bg-accent-indigo-20 border border-accent-indigo-30 text-gray-200'
                  ...
                  <div className="bg-dark-700/60 border border-dark-600 rounded-lg px-2.5 py-2 max-w-[60%]">
                    <div className="flex items-center gap-1">
                      {[0, 0.15, 0.3].map(d => (
                        <span
                          key={d}
                          className="w-1.5 h-1.5 rounded-full bg-accent-indigo-60 animate-bounce"
                ...
                <div className="px-3 pb-1.5 flex flex-wrap gap-1">
                  {QUICK_CHIPS.map(chip => (
                    <button
                      ...
                      disabled={loading}
                      className="text-[10px] px-2 py-0.5 rounded-full border border-dark-500 text-gray-500 hover:border-accent-indigo-50 hover:text-indigo-400 transition-colors"
                    >
                      {chip.label}
                    </button>
                  ))}
                ...
                <div className="flex gap-1.5">
                  <textarea
                    ...
                    className={clsx(
                      ...
                      'focus:border-accent-indigo-60 transition-colors placeholder-gray-600',
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      aiEnabled && !loading && input.trim()
                        ? 'bg-indigo-500 hover:bg-accent-indigo-80 text-white'
