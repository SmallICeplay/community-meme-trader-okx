...
const SUB_TABS = {
  ranking: [
    ...
    { id: 'community',   label: '社区榜' },
  ],
  follow: [
    { id: 'dashboard',  label: '我的跟单' },
...
export default function App() {
  const [topTab, setTopTab] = useState('ranking')
  const [subTab, setSubTab] = useState('leaderboard')

  const handleTopTab = (id) => {
    setTopTab(id)
    setSubTab(SUB_TABS[id][0].id)
  }
  ...
  useEffect(() => {
    window.__switchTab = (id) => {
      // 根据 subtab id 自动判断属于哪个 topTab
      for (const [top, subs] of Object.entries(SUB_TABS)) {
        if (subs.some(s => s.id === id)) { setTopTab(top); setSubTab(id); return }
      }
      setSubTab(id)
    }
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex flex-col">
      ...
      <header className="border-b border-dark-600 bg-dark-800 sticky top-0 z-30 shrink-0">
        <div className="px-3 md:px-4 h-14 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 min-w-0 shrink-0">
            <img src="/logo-icon.png" alt="HODLO.AI" className="h-8 w-auto" />
            <div className={clsx(
              'w-2 h-2 rounded-full hidden sm:block ml-1',
              connected ? 'bg-indigo-500' : 'bg-gray-600'
            )} />
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              title="钱包管理"
              className="px-3 py-1.5 text-xs font-medium rounded-lg bg-dark-700 text-gray-300 border border-dark-500 hover:bg-dark-600 hover:text-gray-200 transition-all"
            >
              钱包
            </button>
            {/* 配置按钮 */}
            <button
              ...
              title="配置"
              className="px-4 py-1.5 text-sm font-medium rounded-lg bg-dark-700 text-gray-300 border border-dark-500 hover:bg-dark-600 hover:text-gray-200 transition-all"
            >
              配置
            </button>
            {/* 管理员入口 */}
            ...
            <button
              ...
              className={clsx(
                ...
                muted
                  ...
                  : 'border-dark-500 text-gray-300 bg-dark-700 hover:bg-dark-600'
              )}
            >
              {muted ? '🔕' : '🔔'}
            </button>
      ...
      {/* 状态横幅 */}
      {!botEnabled ? (
        <div className="bg-dark-700/50 border-b border-dark-600 px-3 py-1.5 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="text-xs px-2.5 py-1 rounded border border-dark-500 text-gray-300 hover:bg-dark-600 transition-colors whitespace-nowrap shrink-0"
          >启用</button>
        </div>
      ) : autoBuyEnabled ? (
        <div className="bg-dark-800/80 border-b border-dark-600 px-3 py-1.5 flex items-center gap-1.5 text-gray-400 text-xs">
          <span>⚡</span>
          <span className="hidden sm:inline">自动购买已开启 — 过滤通过的 CA 将自动买入</span>
          <span className="sm:hidden">自动买入</span>
        </div>
      ) : (
        <div className="bg-dark-800/80 border-b border-dark-600 px-3 py-1.5 flex items-center gap-1.5 text-gray-400 text-xs">
          <span className="text-gray-500">●</span>
          <span className="hidden sm:inline">跟单模式 — 仅对已配置跟单的喊单人执行买入</span>
      ...
      <div className="flex flex-1 md:overflow-hidden">
        ...
        <div className="flex-1 md:overflow-y-auto min-w-0">
          <div className="px-3 md:px-4 py-3 md:py-4">
            ...
            {/* 上级 Tab 导航 */}
            <div className="flex gap-0.5 mb-2 border-b border-dark-600 overflow-x-auto scrollbar-none -mx-3 md:mx-0 px-3 md:px-0">
              {TOP_TABS.map(t => (
                <button
                  key={t.id}
            ...
            {/* 下级 Tab 导航 */}
            <div className="flex gap-0.5 mb-4 md:mb-5 border-b border-dark-700 overflow-x-auto scrollbar-none -mx-3 md:mx-0 px-3 md:px-0">
              {SUB_TABS[topTab].map(t => (
                <button
                  key={t.id}
                  onClick={() => setSubTab(t.id)}
                  className={clsx(
                    'px-3 md:px-4 py-1.5 text-xs font-medium border-b-2 transition-colors -mb-px whitespace-nowrap shrink-0',
                    subTab === t.id
                      ? 'border-gray-300 text-gray-200'
                      : 'border-transparent text-gray-500 hover:text-gray-300'
                  )}
                >
                  ...
                  {t.id === 'positions' && posCount > 0 && (
                    <span className="ml-1 text-xs bg-dark-600 text-gray-300 px-1.5 py-0.5 rounded-full">
                      {posCount}
                    </span>
                  )}
                </button>
              ...
              <div className="space-y-4">
                {stats && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
                    ...
                    <MiniStat label="盈利" value={stats.win_trades} color="text-indigo-400" />
                    ...
                    <MiniStat
                      ...
                      color={stats.total_pnl_usdt >= 0 ? 'text-indigo-400' : 'text-accent-red'}
                    />
                    ...
                    <MiniStat
                      ...
                      value={stats.total_pnl_usdt != null && stats.total_gas_usd != null
                        ...
                        : '—'}
                      color={(stats.total_pnl_usdt - stats.total_gas_usd) >= 0 ? 'text-indigo-400' : 'text-red-400'}
                    />
      ...
      <button
        ...
        title="Agent 日志"
        className="fixed bottom-4 right-4 z-50 w-12 h-12 rounded-full bg-dark-600 hover:bg-dark-500 text-white shadow-lg shadow-dark-900/60 flex items-center justify-center transition-all hover:scale-110 border border-dark-500"
      ...
      {showLiveLog && (
        <div 
          className="fixed inset-0 z-40 bg-dark-900/80"
          onClick={() => setShowLiveLog(false)}
        >
          <div 
            className="absolute right-0 top-0 h-full max-w-full bg-dark-850 border-l border-dark-600 flex flex-col shadow-2xl"
            onClick={e => e.stopPropagation()}
            style={{ width: '18rem', height: '70vh', top: 'auto', bottom: '4rem' }}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-dark-600">
              <span className="text-sm font-bold text-white">Agent 日志</span>
              <button
                onClick={() => setShowLiveLog(false)}
                className="w-7 h-7 flex items-center justify-center rounded-lg bg-dark-700 text-gray-400 hover:text-white hover:bg-dark-600 transition-all text-sm"
              >
                ✕
              </button>
            </div>
      ...
      {/* ── 配置面板（弹窗覆盖层） ────────────────────────────── */}
      {showConfig && (
        <div className="fixed inset-0 z-50 bg-dark-900/90 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">配置</h2>
              <button
                onClick={() => setShowConfig(false)}
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-dark-700 text-gray-400 hover:text-white hover:bg-dark-600 transition-all"
              >
                ✕
              </button>
            </div>
      ...
      {showWallet && (
        <div className="fixed inset-0 z-50 bg-dark-900/90 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">钱包管理</h2>
              <button
      ...
      <div className="flex gap-1 border-b border-dark-700">
        {DATA_SUBTABS.map(t => (
          ...
          >
            {t.label}
            {t.id === 'positions' && posCount > 0 && (
              <span className="ml-1.5 text-xs bg-dark-600 text-gray-300 px-1.5 py-0.5 rounded-full">{posCount}</span>
            )}
          </button>
        ))}
      </div>
          ...
          {STAT_PERIODS.map(p => (
            <button
              ...
              className={clsx(
                ...
                statPeriod === p.key
                  ? 'border-gray-400 text-gray-200 bg-dark-600 font-semibold'
                  : 'border-dark-500 text-gray-600 hover:text-gray-300 hover:border-gray-500'
              )}
            >{p.label}</button>
          ))}
          {statsLoading && (
            <span className="w-1.5 h-1.5 rounded-full bg-gray-400 animate-ping ml-1" />
          )}
        ...
        {[1, 7, 14, 30].map(d => (
          <button
            ...
            className={clsx(
              ...
              days === d
                ? 'border-gray-400 text-gray-200 bg-dark-600'
      ...
      {todayPnl !== null && (
        <>
          ...
          <span className="flex flex-col items-center shrink-0">
            ...
            <span className={clsx('text-xs font-mono font-bold', todayPnl >= 0 ? 'text-indigo-400' : 'text-accent-red')}>
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="flex flex-col items-center">
        ...
        <Num val={pnlStr} rollKey={pnlKey} color={todayPnl === null ? 'text-gray-600' : todayPnl >= 0 ? 'text-indigo-400' : 'text-accent-red'} />
      </div>
      <div className="w-px h-6 bg-dark-500" />
      <div className="flex flex-col items-center">
  ...
  return (
    <div className="flex items-end gap-px h-3.5">
      {BARS.map((h, i) => (
        <div
          key={i}
          className="w-px bg-indigo-500 rounded-full pulse-bar"
          style={{
            height: `${h * 100}%`,
            '--dur': `${0.6 + i * 0.1}s`,
    ...
    <div className="bg-dark-800 rounded-xl border border-dark-600 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => setCollapsed(v => !v)}>
          <span className={clsx('w-1.5 h-1.5 rounded-full shrink-0', fresh ? 'bg-indigo-500 animate-ping' : 'bg-gray-700')} />
    ...
    <div className="bg-dark-800 rounded-xl border border-dark-600 p-4 stat-enter" style={{ animationDelay: `${index * 80}ms` }}>
      ...
      <div className="flex items-center gap-3">
        ...
        <svg width="44" height="44" className="shrink-0 -ml-1">
          ...
          <circle cx="22" cy="22" r={R} fill="none" stroke={winRate >= 50 ? '#9ca3af' : winRate >= 30 ? '#facc15' : '#ff4466'}
      ...
      <div className="mt-1.5 h-1 bg-dark-600 rounded-full overflow-hidden">
        <div className="h-full rounded-full bar-fill transition-all duration-1000"
          ...
          style={{
            ...
            backgroundColor: winRate >= 50 ? '#9ca3af' : winRate >= 30 ? '#facc15' : '#ff4466',
          }} />
      </div>
    </div>
  )
...
function PnlCard({ stats, index, period = 'all' }) {
  ...
  return (
    <div className="bg-dark-800 rounded-xl border border-dark-600 p-4 stat-enter" style={{ animationDelay: `${index * 80}ms` }}>
      ...
      <div key={rollKey} className={clsx('text-2xl font-bold font-mono count-roll tabular-nums', isPos ? 'text-indigo-400' : 'text-accent-red')}>
        {netStr}
      </div>
      {/* 交易P&L vs Gas 拆解 */}
      <div className="mt-2 space-y-1">
        ...
        <div className="h-1 bg-dark-600 rounded-full overflow-hidden flex">
          <div key={rollKey} className="h-full rounded-l-full bar-fill"
            style={{ width: `${100 - gasPct}%`, backgroundColor: pnl >= 0 ? '#9ca3af' : '#ff4466', opacity: 0.8 }} />
                ...
                <div key={p.id} className="flex items-center gap-1.5">
                  ...
                  <div className="flex-1 h-1 bg-dark-600 rounded-full overflow-hidden">
                    <div className="h-full rounded-full"
                      style={{
                        width: `${barW}%`,
                        backgroundColor: pct >= 0 ? '#9ca3af' : '#ff4466',
                        opacity: 0.8,
        ...
        <div className="h-1 bg-dark-600 rounded-full overflow-hidden">
          <div
            ...
            style={{
              ...
              backgroundColor: gasRatio < 20 ? '#9ca3af' : gasRatio < 50 ? '#facc15' : '#ff4466',
            }}
          />
        </div>
      </div>
    ...
    <div className="bg-dark-800 rounded-xl border border-dark-600 p-4 stat-enter" style={{ animationDelay: `${index * 80}ms` }}>
      ...
      <div className="h-1.5 bg-dark-600 rounded-full overflow-hidden flex mb-2">
        <div
          ...
          style={{ width: `${winPct}%`, backgroundColor: '#9ca3af', opacity: 0.85 }}
        />
        <div
          ...
          {Object.entries(PERIOD_LABELS).map(([k, v]) => (
            <button
              ...
              className={clsx(
                ...
                period === k ? 'bg-dark-600 text-gray-200' : 'text-gray-600 hover:text-gray-400'
              )}
            >{v}</button>
          ))}
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-1 text-center border-t border-dark-600 pt-1.5">
            ...
            <div>
              ...
              <div className="text-xs font-mono text-indigo-400">{data.bought}</div>
            </div>
          </div>
        </>
      ) : (
        ...
        {shown.map(t => {
          ...
          return (
            <div key={t.id} className="flex items-center gap-1.5 md:gap-2 group">
              ...
              {/* 净盈亏 */}
              <span className={clsx('font-mono text-[11px] w-12 md:w-16 text-right shrink-0', netPnl >= 0 ? 'text-indigo-400' : 'text-red-400')}>
