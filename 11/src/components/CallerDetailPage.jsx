  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="border border-dark-600 rounded-xl bg-dark-800/60 px-5 py-4">
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
              {[
                ...
                { label: '今日收益率', value: item.ca_count > 0 ? `+${((item.total_multiplier / item.ca_count) * 100).toFixed(1)}%` : '—', color: 'text-indigo-400' },
              ].map(s => (
        ...
        <div className="space-y-4">
          {editing && editForm ? (
            <div className="border border-accent-indigo-30 rounded-xl bg-dark-800/60 px-5 py-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setF('enabled', !editForm.enabled)}
                  className={clsx('relative w-10 h-5 rounded-full transition-colors shrink-0',
                    editForm.enabled ? 'bg-indigo-500' : 'bg-dark-600')}>
                    ...
                    {[0.05,0.1,0.2,0.5,1].map(v => (
                      <button key={v} onClick={() => setF('buy_amount', v)}
                        className={clsx('text-xs px-2 py-0.5 rounded border transition-colors',
                          editForm.buy_amount === v ? 'border-accent-indigo-60 text-indigo-400 bg-accent-indigo-10' : 'border-dark-500 text-gray-500 hover:text-gray-300'
                        )}>{v}U</button>
                    ))}
                  </div>
                  <input type="number" step="0.01" min="0.01" value={editForm.buy_amount}
                    onChange={e => setF('buy_amount', parseFloat(e.target.value)||0.1)}
                    className="w-full bg-dark-700 border border-dark-500 rounded px-3 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-accent-indigo-60" />
                </div>
                  ...
                  <div className="flex gap-1 flex-wrap mb-1">
                    {[30,60,120,240].map(v => (
                      <button key={v} onClick={() => setF('max_hold_min', v)}
                        className={clsx('text-xs px-2 py-0.5 rounded border transition-colors',
                          editForm.max_hold_min === v ? 'border-accent-indigo-60 text-indigo-400 bg-accent-indigo-10' : 'border-dark-500 text-gray-500 hover:text-gray-300'
                        )}>{v}分</button>
                    ))}
                  </div>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button onClick={handleSave} disabled={saving}
                  className="flex-1 py-1.5 text-xs rounded bg-accent-indigo-90 hover:bg-indigo-500 text-white font-medium transition-colors">
                  {saving ? '保存中…' : '保存'}
                </button>
              </div>
            </div>
            ...
            <div className="border border-green-800/40 rounded-xl bg-green-900/10 px-5 py-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: '买入金额', value: `${followCfg.buy_amount}U`, color: 'text-indigo-400' },
            ...
            <div className="border border-dark-600 rounded-xl bg-dark-800/60 px-5 py-6 flex flex-col items-center justify-center gap-3 text-center">
              ...
              <button onClick={startEdit}
                className="text-xs px-4 py-2 rounded border border-accent-indigo-50 text-indigo-400 bg-accent-indigo-10 hover:bg-accent-indigo-20 transition-colors">
                + 设置跟单
              </button>
            </div>
          )}
